-- Update get_top_users_by_messages to use daily_user_stats
CREATE OR REPLACE FUNCTION get_top_users_by_messages(
  p_guild_id BIGINT,
  p_days INTEGER DEFAULT 30,
  p_limit INTEGER DEFAULT 10,
  p_start_date TEXT DEFAULT NULL -- Supabase might pass this as TEXT or timestamp, verifying usage in lib/supabase.ts it passed as string/null
)
RETURNS TABLE (
  user_id BIGINT,
  username TEXT,
...
AS $$
  ...
  WHERE s.guild_id = p_guild_id
    AND (p_start_date IS NULL OR s.date >= p_start_date::DATE)
    AND (p_start_date IS NOT NULL OR p_days IS NULL OR s.date >= (CURRENT_DATE - (p_days || ' days')::INTERVAL)::DATE)
    AND u.is_bot = FALSE
...
-- Update get_top_users_by_voice to use daily_user_stats
...
-- Update get_daily_message_stats to use daily_user_stats
...
-- Update get_daily_voice_stats to use daily_user_stats
