...
class HighlightsBuilder:
    """
    Gerador visual dos slides da Retrospectiva / Destaques do Ano (BMIA Wrapped)
    Renderiza artes em alta fidelidade 1300x850 px com Playwright.
    """

    @staticmethod
    async def _fetch_avatar_data_uri(avatar_url: Optional[str], username: str) -> str:
        """Converte o avatar para data URI em Base64."""
        default_svg = f"""<svg xmlns="http://www.w3.org/2000/svg" width="160" height="160" viewBox="0 0 160 160">
            <rect width="160" height="160" fill="#131722"/>
            <text x="50%" y="54%" dominant-baseline="middle" text-anchor="middle" fill="#00f0ff" font-size="64" font-weight="bold" font-family="sans-serif">{username[:2].upper() if username else 'BM'}</text>
        </svg>"""
...
<body>
    ...
    <div class="footer">Navegue pelas páginas abaixo usando as setas ou o menu de categorias • BMIA Esports</div>
</body>
</html>"""
    ...
    <style>
        * {{ margin: 0; padding: 0; box-sizing: border-box; }}
        body {{
            ...
            background: #06080d;
            background-image: 
                radial-gradient(circle at 50% 10%, {theme_color}22 0%, transparent 60%),
                radial-gradient(circle at 80% 90%, rgba(176, 38, 255, 0.12) 0%, transparent 50%),
                radial-gradient(circle at 50% 50%, rgba(15, 23, 42, 0.95) 0%, #06080d 100%);
            font-family: 'Rajdhani', sans-serif;
            color: #ffffff;
            display: flex;
        ...
        .top-glow {{
            position: absolute; top: 0; left: 0; right: 0; height: 4px;
            background: linear-gradient(90deg, {theme_color}, #b026ff, #ffd700, {theme_color});
            box-shadow: 0 0 25px {theme_color}aa;
        }}
        .header {{
            display: flex;
            align-items: center;
        ...
        .header-icon {{
            ...
            box-shadow: 0 0 25px {theme_color}66;
        }}
        .title {{
            font-family: 'Orbitron', sans-serif;
            font-size: 36px;
        ...
        .pillar-1 {{ order: 2; width: 340px; }}
        .pillar-2 {{ order: 1; width: 280px; }}
        .pillar-3 {{ order: 3; width: 280px; }}
        ...
        .p1-avatar {{ width: 140px; height: 140px; border: 4px solid #ffd700; box-shadow: 0 0 30px rgba(255,215,0,0.8); }}
        .p2-avatar {{ width: 110px; height: 110px; border: 3px solid #00f0ff; box-shadow: 0 0 25px rgba(0,240,255,0.7); }}
        .p3-avatar {{ width: 110px; height: 110px; border: 3px solid #b026ff; box-shadow: 0 0 25px rgba(176,38,255,0.7); }}
        ...
        .p1-tag {{ background: #ffd700; color: #000; box-shadow: 0 0 15px #ffd700; }}
        .p2-tag {{ background: #00f0ff; color: #000; box-shadow: 0 0 15px #00f0ff; }}
        .p3-tag {{ background: #b026ff; color: #fff; box-shadow: 0 0 15px #b026ff; }}
        ...
        .pedestal-1 {{
            height: 250px;
            background: linear-gradient(180deg, rgba(255,215,0,0.2) 0%, rgba(15,23,42,0.95) 100%);
            border-top: 3px solid #ffd700;
            box-shadow: 0 0 40px rgba(255,215,0,0.25);
        }}
        .pedestal-2 {{
            height: 190px;
            background: linear-gradient(180deg, rgba(0,240,255,0.18) 0%, rgba(15,23,42,0.95) 100%);
            border-top: 3px solid #00f0ff;
            box-shadow: 0 0 30px rgba(0,240,255,0.2);
        }}
        .pedestal-3 {{
            height: 150px;
            background: linear-gradient(180deg, rgba(176,38,255,0.18) 0%, rgba(15,23,42,0.95) 100%);
        ...
        .member-val {{
            ...
            font-weight: 700;
            color: {theme_color};
            text-shadow: 0 0 10px {theme_color}88;
        }}

        /* Honorable mentions footer */
        .honorable-bar {{
            display: flex;
            align-items: center;
            justify-content: center;
        ...
        .honorable-pos {{ font-family: 'Orbitron', sans-serif; font-weight: 800; color: #94a3b8; }}
        .honorable-name {{ font-weight: 600; color: #e2e8f0; }}
        .honorable-val {{ font-weight: 700; color: #38bdf8; }}
    </style>
</head>
<body>
    <div class="top-glow"></div>
    ...
    <div class="podium-stage">
        ...
        <div class="pillar pillar-2">
            <div class="avatar-wrap">
                <img class="p2-avatar" src="{p2['avatar_uri']}" alt="{p2['name']}">
                <div class="rank-tag p2-tag">2º LUGAR</div>
            </div>
            <div class="pedestal pedestal-2">
                <div class="member-name">{p2['name']}</div>
                <div class="member-val">{p2['val_str']} {unit_label}</div>
            </div>
        </div>
        ...
        <div class="pillar pillar-1">
            <div class="avatar-wrap">
                <img class="p1-avatar" src="{p1['avatar_uri']}" alt="{p1['name']}">
                <div class="rank-tag p1-tag">👑 CAMPEÃO</div>
            </div>
            <div class="pedestal pedestal-1">
                <div class="member-name" style="font-size: 24px; color: #ffd700;">{p1['name']}</div>
                <div class="member-val" style="font-size: 28px; color: #ffd700;">{p1['val_str']} {unit_label}</div>
            </div>
        </div>
        ...
        <div class="pillar pillar-3">
            <div class="avatar-wrap">
                <img class="p3-avatar" src="{p3['avatar_uri']}" alt="{p3['name']}">
                <div class="rank-tag p3-tag">3º LUGAR</div>
            </div>
            <div class="pedestal pedestal-3">
                <div class="member-name">{p3['name']}</div>
                <div class="member-val">{p3['val_str']} {unit_label}</div>
            </div>
        </div>
    </div>

    <div class="honorable-bar">
        {honorable_cards_html}
    </div>
</body>
</html>"""
    ...
    async def generate_media_slide(
        ...
        # Se houver imagem válida de preview
...
<body>
    ...
    async def generate_all_slides_files(
        ...
        """
        Gera simultaneamente todos os slides dos Destaques do Ano em paralelo
        e retorna uma lista ordenada de discord.File para envio como Galeria Nativa no Discord.
        """
        if categories is None:
            from commands.stats_commands import HIGHLIGHTS_CATEGORIES
            categories = HIGHLIGHTS_CATEGORIES

        async def render_one(idx: int, cat: Dict[str, Any]):
            cat_id = cat["id"]
            if cat_id == "cover":
                buf = await cls.generate_cover_slide(guild, year, len(categories))
            elif cat_id == "media":
                buf = await cls.generate_media_slide(guild, year, top_clip)
            else:
                winners = highlights_data.get(cat_id, [])
                buf = await cls.generate_category_slide(
                    guild=guild,
                    year=year,
            ...
            file = discord.File(fp=buf, filename=f"destaques_{idx+1:02d}_{cat_id}.png")
            return idx, file

        tasks = [render_one(i, cat) for i, cat in enumerate(categories)]
